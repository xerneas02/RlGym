...
class ZZeerAction(ActionParser):
    def __init__(self):
        super().__init__()
    ...
    def make_lookup_table():
        actions = []
        # Ground
        for throttle in (-1, 0, 1): #accélération
            for steer in (-1, 0, 1):  # direction
                for boost in (0, 1):  # boost
                    for handbrake in (0, 1): #dérapage
                        if boost == 1 and throttle != 1:
                            continue
                        actions.append([throttle or boost, steer, 0, steer, 0, 0, boost, handbrake])
        # Aerial
        for pitch in (-1, 0, 1):  # angle hauteur
            for yaw in (-1, 0, 1):  #angle horizontale
                for roll in (-1, 0, 1):  #tonneau
                    for jump in (0, 1):  #jump
                        for boost in (0, 1):
